"""The command line entry point for RefineNovo."""
import datetime
import functools
import logging
import os
...
logger = logging.getLogger("RefineNovo")
...
@click.option(
    ...
    required=True,
    default="denovo",
    help="\b\nThe mode in which to run RefineNovo:\n"
...
def main(
    ...
    if output is None:
        output = os.path.join(
            os.getcwd(),
            f"RefineNovo_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}",
        )
    else:
    ...
    # Run RefineNovo in the specified mode.
    if mode == "denovo":
        logger.info("Predict peptide sequences with RefineNovo.")
        writer = None
        # writer.set_metadata(
        ...
        logger.info("Evaluate a trained RefineNovo model.")
        model_runner.evaluate(peak_path, model, config)
    elif mode == "train":
        logger.info("Train the RefineNovo model.")
